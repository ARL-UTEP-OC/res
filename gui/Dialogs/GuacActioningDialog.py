    ...
    def run(self):
        ...
        try:
            ...
            if self.actionname == "Add":
                ...
                cmd = "guac create " + self.configname + " --hostname " + str(self.args[0]) + " --username " + str(self.args[1]) + " --password " + str(self.args[2]) + " --maxConnections " + str(self.args[3]) + " --maxConnectionsPerUser " + str(self.args[4]) + " --width " + str(self.args[5]) + " --height " + str(self.args[6]) + " --bitdepth " + str(self.args[7]) + " --creds_file " + creds_file + " --itype " + str(self.args[9]) + " --name " + str(self.args[10])
            if self.actionname == "Remove":
                ...
                if str(self.args[3]).strip() != "":
                    ...
                    if " " in creds_file:
                        creds_file = "\""+creds_file+"\""
                cmd = "guac remove " + self.configname + " --hostname " + str(self.args[0]) + " --username " + str(self.args[1]) + " --password " + str(self.args[2]) + " --creds_file " + creds_file + " --itype " + str(self.args[4]) + " --name " + str(self.args[5])
            if self.actionname == "Clear":
                if len(self.args) != 3:
                    ...
                    self.status = -1
                    return None
                cmd = "guac " + " clear --hostname " + str(self.args[0]) + " --username " + str(self.args[1]) + " --password " + str(self.args[2])
...
class GuacActioningDialog(QDialog):
    def __init__(self, parent, configname, actionname, args):
        logging.debug("GuacActioningDialog(): instantiated")
        super(GuacActioningDialog, self).__init__(parent)     
    ...
    def exec_(self):
        ...
        t.start()
        result = super(GuacActioningDialog, self).exec_()
