...
class WatchRetrieveThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            logging.debug("watchRetrieveStatus(): running: keycloak refresh")
            #e.execute("keycloak refresh")
            if len(self.args) != 3:
                ...
                self.status = -1
                return None
            #format: "keycloak refresh <ip> <user> <pass> <path>"
            cmd = "keycloak " + " refresh " + self.configname + " --hostname " + str(self.args[0]) + " --username " + str(self.args[1]) + " --password " + str(self.args[2])
            e.execute(cmd)
...
class KeycloakRetrievingDialog(QDialog):
    def __init__(self, parent, configname, args):
        logging.debug("KeycloakRetrievingDialog(): instantiated")
        super(KeycloakRetrievingDialog, self).__init__(parent)     
        self.configname = configname
        self.args = args
