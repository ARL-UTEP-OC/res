...
from gui.Dialogs.GuacOpeningDialog import GuacOpeningDialog
from gui.Dialogs.GuacRetrievingDialog import GuacRetrievingDialog
...
from gui.Dialogs.GuacActioningDialog import GuacActioningDialog
import logging
from engine.Configuration.UserPool import UserPool

class GuacActionDialog(QDialog):

    def __init__(self, parent, configname, actionname, experimentHostname, rdpBrokerHostname="", users_file="", itype="", name=""):
        logging.debug("GuacActionDialog(): instantiated")
        super(GuacActionDialog, self).__init__(parent)
        self.parent = parent
    ...
    def createFormGroupBox(self):
        ...
        self.layout.addRow(QLabel("Guacamole Server URL:"), self.hostnameLineEdit)
        mgmusername = ""
        mgmpassword = ""
    ...
    def exec_(self):
        logging.debug("GuacActionDialog(): exec_() instantiated")
        result = super(GuacActionDialog, self).exec_()
        if str(result) == str(1):
            ...
            if self.actionname == "Refresh":
                ...
                crd = GuacRetrievingDialog(self.parent, self.configname, self.args).exec_()
                return crd
            elif self.actionname == "Open":
                self.eco.storeConfigRDPBrokerCreds(self.configname, self.usernameLineEdit.text(), self.passwordLineEdit.text())
                ...
                cod = GuacOpeningDialog(self.parent, pathToBrowser, browserArgs, usersToOpen, url).exec_()
                return cod
            else:
                self.eco.storeConfigRDPBrokerCreds(self.configname, self.usernameLineEdit.text(), self.passwordLineEdit.text())
                cad = GuacActioningDialog(self.parent, self.configname, self.actionname, self.args).exec_()
